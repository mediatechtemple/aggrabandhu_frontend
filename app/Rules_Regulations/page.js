...
const FileUploadComponent = () => {
  ...
  // Fetch existing PDF when component mounts
  useEffect(() => {
...
console.log(data);
        if (response.ok && data.filePath) {
          // Set the PDF file path if it exists
          setPdfFilePath(data.filePath);
        } else {
          setErrorMessage('No file available.');
        }
      } catch (error) {
  ...
  return (
    <div className="flex flex-col items-center p-4">
      <h2 className="text-xl mb-4">Rules and regulations</h2>
