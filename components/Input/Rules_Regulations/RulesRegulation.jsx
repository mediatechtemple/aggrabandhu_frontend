...
const RulesRegulation = () => {
        ...
        const postContent = async () => {
            try {
              const response = await fetch('https://agerbandhu-production.up.railway.app/api/rule/', {
                ...
                body: JSON.stringify({ rule: editorRef.current.value }), // Sending content as JSON
              });
        
              if (response.ok) {
                const data = await response.json();
                console.log('Content posted successfully:', data);
                setPostedData(data.rule); // Assuming the API returns the posted rule in the response
        ...
        const getContent = async () => {
            try {
              ...
              if (response.ok) {
                const data = await response.json();
                console.log('Content posted successfully:', data);
                setPostedData(data[0].rule); // Assuming the API returns the posted rule in the response
                setContent(''); // Clear the editor content
              } else {
                console.error('Error posting content:', response.statusText);
              }
    ...
    return (
        <>
        <div>
            <h2 className='text-center bg-blue-500 text-white p-4 text-2xl'>Rules & Regulations</h2>
            ...
            <h3 className="text-lg font-semibold">Posted Rules and Regulations:</h3>
