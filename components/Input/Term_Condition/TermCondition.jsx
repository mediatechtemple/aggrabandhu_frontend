...
const PrivacyPolicy = () => {
    ...
    const handleSubmit = (event) => {
        ...
        const postContent = async () => {
            try {
              const response = await fetch('https://backend.aggrabandhuss.org/api/termcondition/', {
                method: 'POST',
                headers: {
                  'Content-Type': 'application/json',
                },
                body: JSON.stringify({ termcondition: content }), // Sending content as JSON
              });
        
              if (response.ok) {
                const data = await response.json();
                console.log('Content posted successfully:', data);
                setPostedData(data.termcondition); // Assuming the API returns the posted termcondition
        ...
        const getContent = async () => {
            try {
              ...
              if (response.ok) {
                ...
                setPostedData(data[0].termcondition); // Assuming the API returns the policy
              } else {
    ...
    return (
        <>
        <div>
            <h2 className='text-center bg-blue-500 text-white p-4 text-2xl'>Term And Condition</h2>
            ...
            <div className="mt-4 p-4 border border-blue-300 rounded overflow-x-auto">
                <h3 className="text-lg font-semibold">Posted Term And Condition:</h3>
